...
`Registers` は `BC`、`DE`、`HL` を 16 ビット `int` フィールドとして保持しつつ、`B`/`C`、`D`/`E`、`H`/`L` を 16 ビットペアに対するシフト・マスクの `@property` getter/setter として公開します（`Registers.B`/`C` とその同類）。1 バイトの半値を読み書きする各オペコードがディスクリプタプロトコルを通り、ホットな命令パスにコールフレームが増加します。
