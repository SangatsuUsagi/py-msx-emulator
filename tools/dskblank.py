...
Usage:
    python tools/dskblank.py OUTPUT.dsk [--size KB]
...
def main() -> None:
    ...
    parser.add_argument(
        "--size", type=float, default=720.0, help="Image size in KiB (default: 720 = 2DD)"
    )
    parser.add_argument(
    ...
    if total <= 0 or (total % SECTOR_SIZE) != 0:
        print(
            f"error: size {args.size} KiB is not a positive multiple of {SECTOR_SIZE} bytes",
            file=sys.stderr,
        )
        sys.exit(1)
