...
- **FM-PAC（MSX-MUSIC）** — `--fmpac` で有効化するオプションのオーバーレイカートリッジ。プライマリスロット 2 に配置：64 KB バンク切り替え ROM、openMSX 互換のマジック値アンロック方式 8 KB バッテリーバックアップ SRAM（`saves/sram/fmpac.sram` に永続化）、YM2413（OPLL）FM 音源チップ — 9 チャンネル 2 オペレータ FM メロディ合成（内蔵 15 音色 + ユーザー定義音色）、ADSR エンベロープ、リズムモード（バスドラム、スネア、タム、トップシンバル、ハイハット）を PSG/SCC と混合してオーディオ出力
...
- **ステートセーブ/ロード** — stdlib JSON による完全なハードウェアスナップショット（CPU、RAM、VDP、PSG、SCC、FM-PAC/OPLL、マッパーバンク）、セーブごとに PNG スクリーンショットも保存、素早い復帰のための `saves/states/latest.*` シンボリックリンク
