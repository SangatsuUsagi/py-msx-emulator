...
> **このエミュレータの主な目標は、MSX1 上で [KONAMI の沙羅曼蛇（Salamander）](https://ja.wikipedia.org/wiki/%E6%B2%99%E7%BE%85%E6%9B%BC%E8%9B%87_(MSX)) を動かすことです。**
>
...
- **MSX スロットシステム** — 4 ページ × 4 スロットディスパッチ、スロット 0 ページ 0–1 に BIOS ROM、スロット 0 ページ 2（0x8000–0xBFFF）に BIOS と同じディレクトリの `cbios_logo_msx1.rom` があれば自動的にロードされるコンパニオンロゴ ROM、スロット 1 にカートリッジ、スロット 2 にオプションの第 2 カートリッジ、スロット 3 に 32 KB RAM
...
2. アーカイブを展開し、以下のファイルをこのリポジトリの `roms/` ディレクトリにコピーします。
   - `cbios_main_msx1.rom`（必須）
   - `cbios_logo_msx1.rom`（任意 — C-BIOS 起動ロゴを有効にします）
3. CLI はデフォルトで `roms/cbios_main_msx1.rom` を BIOS として使用します。変更する場合は `--biosrom` で指定してください。

`cbios_logo_msx1.rom` がメイン BIOS と同じディレクトリに存在する場合、スロット 0 / ページ 2（0x8000–0xBFFF）のコンパニオンロゴ ROM として自動的にマップされます。存在しない場合もエミュレータは動作しますが、起動ロゴは表示されません。
...
```bash
# MSX BASIC のみ（カートリッジなし）；BIOS は roms/ から自動検出
python .
...
| `--biosrom BIOS_PATH` | `roms/cbios_main_msx1.rom` | メイン BIOS ROM のパス（自動選択されるデフォルトを上書き） |
...
テストスイートは 513 個のテストで構成されており、個々のオペコードやハードウェアレジスタを対象としたユニットテスト、複数コンポーネントを組み合わせた統合テスト、仕様書のシナリオから直接導出したシナリオレベルのテストが含まれます。
...
├── tests/                 # テストスイート — 513 テスト（公開リポジトリには含まれていません）
