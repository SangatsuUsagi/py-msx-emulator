...
このエミュレータは作者の所有する以下のハードウェア、ソフトウェアでのみ検証されています。
...
完全なレジスタファイル（AF、BC、DE、HL、IX、IY、SP、PC、I、R およびシャドウレジスタ）、252 の文書化済みオペコードすべて、CB/DD/ED/FD プレフィックステーブル、非文書化の IXH/IXL/IYH/IYL レジスタアクセスオペコード、マスカブル割り込み（INT モード 1・モード 2）および非マスカブル割り込み（NMI）を備え、T ステート精度でステップ実行します。MSX 向けに **M1 ウェイトステート**（オペコードフェッチごとに +1 T ステート、実機 / openMSX 準拠）を設定可能で、デフォルトは 0（純正 Zilog Z80）です。

- 実装：`msx/cpu/z80.py`、`msx/cpu/opcodes_main.py`、`msx/cpu/registers.py`
- 既知の制限：
  - OTIR/INIR 等のブロック I/O 命令はページ境界をまたぐ場合にサイクル精度でない。
  - R レジスタは実機の M1（オペコードフェッチ）サイクルのみのインクリメントとは異なり、フェッチする全バイト（オペコードもオペランドも）でインクリメントされる。
  - 非文書化フラグビット Y（ビット5）と X（ビット3）はブロック I/O のフラグ処理以外では再現されず、`PUSH AF`/`POP AF`/`EX AF,AF'` でもラウンドトリップされない。
...
- 既知の制限：
  - フレーム途中でのレジスタ変更タイミングはエミュレートされていない：スクリーンモード・色・スプライト設定をフレーム途中で変更するゲームは、変更後のスキャンラインからではなくフレーム全体にその新しい状態が適用された結果を見ることになる。非文書化のスプライトオーバーフロー動作もエミュレートされていない。
...
16 レジスタ、3 トーンチャンネル、ノイズチャンネル、8 波形エンベロープジェネレータ、実測ベースの振幅テーブル（理想化された対数カーブではなく、ayumi のオシロスコープ実測 AY-3-8910 DAC カーブから導出）、44100 Hz PCM サンプル出力（1 フレームあたり 735 サンプル）。**サブフレーム・ソフトウェア PCM** 再現はボリュームレジスタ書き込みをサイクル単位でタイムスタンプし、フレーム内のサンプル位置に再生します。トーンジェネレータを出力サンプルごとに積分するため、period-0 の超音波 PCM キャリアはデューティ平均に帯域制限され、full/zero のチョッピング（エイリアシング）に陥りません。
...
フラット（バンク切り替えなし）、ASCII8、ASCII16、Konami、KonamiSCC、Majutsushi（DAC）、ASCII8SRAM2/8、ASCII16SRAM2/8、R-Type、固定配置マッパー（Page2/0x4000/0x8000）、KoeiSRAM32、GameMaster2 を SHA1 ベースの ROM データベースから自動検出します。`--mapper` オプションで上書き可能です。
...
- 実装：`msx/fdc/wd2793.py`、`msx/fdc/interface.py`
- 既知の制限：
...
- 実装：`msx/fdc/tc8566af.py`、`msx/fdc/interface.py`
- 既知の制限：
  ...
  - 現状出荷しているマシンはすべてドライブ1台構成のため、複数ドライブ動作は未検証です。実行時ディスク入れ替えの中断処理はドライブ0のみを正しく追跡します。ドライブ選択も、Control Register 0の永続的な選択状態ではなく各コマンド自体のパラメータバイトから毎回決定しています（いずれも実機のDISK ROM通信パターン上はシングルドライブ動作と一致します）。
...
| 既知の制限 | <ul><li>NYYRIKKI/Prodatron 拡張プロトコル（高解像度、ホイール、追加ボタン）は未実装。</li><li>絶対座標指定は未実装。</li><li>実機マウスのクリック長押しによるジョイスティックエミュレーションモードは未実装。</li></ul> |
...
- 既知の制限：オーディオデバイスが開かれていない場合（ヘッドレス/`--benchmark` 実行時など）、PSG/SCC/OPLL はサンプル生成の呼び出しを一切受け取らないため、その間はエンベロープ/LFO などの内部状態が進行しない。
...
`--benchmark` CLI オプションで計測しています。ヘッドレス・無制限速度（`FrameTimer` によるペーシングなし）で実行し、`--resume` により保存済みの中盤シーンから開始します（BIOS 起動時間は計測から除外）。CPU エミュレーションとオフスクリーンバッファへの VDP 描画は毎フレーム実行され、インタラクティブ再生時と同じ描画コストを負担します。ただし `--benchmark` は SDL ウィンドウの作成・オーディオサンプル生成・テクスチャアップロード/ブリットを一切行わないため、実際のインタラクティブセッションはここに示す生の数値よりもいくらか遅くなります。
...
![Raspberry Pi 5 でのベンチマーク推移](assets/bench-history-rpi5.png)
